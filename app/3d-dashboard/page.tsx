...
export default function ThreeDHome() {
  ...
  const fetchVideos = async () => {
    const { data, error } = await supabase
      .from('shorts_queue')
      .select('*')
      .eq('data_json->>type', '3d') // Only show 3D videos
  ...
  const handleGenerate = async () => {
    ...
    setMessage(null);
    try {
      const response = await fetch('/api/generate-3d-topic', { 
        method: 'POST',
      ...
      if (response.ok && data.success) {
        setMessage({ type: 'success', text: '3D Video topic generated and queued!' });
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this 3D video and its files?')) return;
    try {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-lg p-8 border border-gray-100 dark:border-gray-700 flex flex-col md:flex-row justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-blue-600 dark:text-blue-400 mb-2">3D Shorts Dashboard</h1>
            <p className="text-gray-500 dark:text-gray-400">Control Panel for 3D Video Generation</p>
          </div>
          <div className="flex flex-col md:flex-row gap-4 mt-6 md:mt-0 items-center">
            ...
            <Link href="/" className="px-6 py-3 rounded-xl bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white font-medium hover:bg-gray-300 dark:hover:bg-gray-600 transition-all text-center">
              2D View
            </Link>
            <button
            ...
            >
              {loading ? 'Generating 3D...' : 'Generate New 3D Topic'}
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          {videos.length === 0 && (
            <div className="text-center p-12 text-gray-500 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700">
              No 3D videos yet. Generate one above!
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 space-y-4">
        <div className="flex items-start justify-between">
          <div>
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white">{video.topic || 'Untitled'} (3D)</h3>
          </div>
